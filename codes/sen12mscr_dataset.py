...
class SEN12MSCR(Dataset):
    def __init__(self, opts, root, split="all", region='all', sample_type='pretrain'):
    ...
    def __getitem__(self, pdx):  # get the triplet of patch with ID pdx
        ...
        # added to cloud mask   (new!!!)
        if self.opts.is_use_cloudmask:
            cloud_mask = get_cloud_cloudshadow_mask(s2_cloudy.numpy(), self.opts.cloud_threshold)
            cloud_mask = torch.from_numpy((cloud_mask != 0).astype('float32'))

        if self.opts.is_use_cloudmask:
            cloud_mask = torch.from_numpy(cloud_mask)

        # added to crop and augmentation
        if self.opts.load_size - self.opts.crop_size > 0 :
            ...
            s1        = s1[...,y:y+self.opts.crop_size,x:x+self.opts.crop_size]
            s2        = s2[...,y:y+self.opts.crop_size,x:x+self.opts.crop_size]
            s2_cloudy = s2_cloudy[...,y:y+self.opts.crop_size,x:x+self.opts.crop_size]
        
            if self.opts.is_use_cloudmask:
                cloud_mask = cloud_mask[y:y+self.opts.crop_size,x:x+self.opts.crop_size]

        results = {
            'cloudy_data': s2_cloudy,
            'cloudfree_data': s2,
            'SAR_data': s1,
            'file_name': os.path.basename(self.paths[pdx]['S2'])
        }
        if self.opts.is_use_cloudmask:
